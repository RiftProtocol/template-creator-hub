...
import { Shield, Lock, Zap, Users, Code, ChevronDown } from "lucide-react";
import { Header } from "@/components/layout/Header";
import { Footer } from "@/components/layout/Footer";
import { VideoBackground } from "@/components/layout/VideoBackground";
import { RIFT_FEATURES, PRIVACY_POOLS, CRYPTO_PARAMS } from "@/lib/protocol/constants";
          ...
          <div className="max-w-6xl mx-auto">
            ...
            <section className="mb-16">
              <h2 className="text-2xl font-bold mb-8 flex items-center gap-2 text-white">
                <Shield className="w-6 h-6 text-[#FFCC00]" /> Core Innovations
              </h2>
            ...
            <section className="mb-16">
              <h2 className="text-2xl font-bold mb-8 flex items-center gap-2 text-white">
                <Code className="w-6 h-6 text-[#FFCC00]" /> Cryptographic Design
              </h2>
                ...
                <div className="bg-white/5 border border-white/10 rounded-2xl p-6 backdrop-blur-sm">
                  ...
                  <ul className="space-y-2 text-sm text-white/60">
                    <li className="flex items-center gap-2">
                      <span className="text-[#FFCC00]">✓</span> On-chain nullifier tracking prevents double-spend
                    </li>
                    <li className="flex items-center gap-2">
                      <span className="text-[#FFCC00]">✓</span> Root history enables async withdrawals
                    </li>
                    <li className="flex items-center gap-2">
                      <span className="text-[#FFCC00]">✓</span> Groth16 proofs verify in &lt;1ms
                    </li>
                    <li className="flex items-center gap-2">
                      <span className="text-[#FFCC00]">✓</span> 128-bit security level
                    </li>
                  </ul>
                </div>
              </div>
            ...
            <section className="mb-16">
              <h2 className="text-2xl font-bold mb-8 flex items-center gap-2 text-white">
                <Code className="w-6 h-6 text-[#FFCC00]" /> Smart Contracts (Open Source)
              </h2>
              <p className="text-white/60 mb-6">
                All contracts are in the codebase at <code className="text-[#FFCC00] bg-white/5 px-2 py-1 rounded">contracts/rift-mixer/programs/</code>. 
            ...
            <section className="text-center">
              <div className="bg-[#FFCC00]/10 border border-[#FFCC00]/30 rounded-2xl p-8 backdrop-blur-sm">
                ...
                <div className="flex flex-wrap justify-center gap-4">
                  <Link 
                    to="/mix" 
                    className="bg-[#FFCC00] text-black px-6 py-3 rounded-xl font-semibold hover:bg-[#FFCC00]/90 transition-colors"
                  >
                    Launch Mixer
                  </Link>
                  <Link 
      ...
      <div className="bg-white/5 border border-white/10 rounded-2xl overflow-hidden backdrop-blur-sm">
        <CollapsibleTrigger className="w-full p-6 flex items-center justify-between hover:bg-white/5 transition-colors">
          <div className="text-left">
            ...
            <code className="text-xs text-[#FFCC00] mt-2 block">{path}</code>
          </div>
